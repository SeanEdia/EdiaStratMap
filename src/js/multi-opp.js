...
// Strip ALL runtime/derived fields from accounts for clean repo download
export function stripRuntimeFields(accounts) {
  const RUNTIME_FIELDS = new Set([
    // Precomputed search fields (regenerated by precomputeSearchFields)
    '_nameLc', '_stateLc', '_regionLc',
    // Cross-link fields (regenerated by crossLinkCustomers)
    'is_customer', 'customer_name', 'also_account',
    'parent_district', 'sibling_customer_count', 'district_is_customer', 'expand_opp',
    // Derived opp summary fields (regenerated by deriveOppSummary)
    'opp_count', 'opp_stage', 'opp_forecast', 'opp_areas', 'opp_acv',
    'opp_probability', 'opp_next_step', 'opp_contact', 'opp_contact_title',
    'opp_sdr', 'opp_champion', 'opp_economic_buyer', 'opp_competition',
    'opp_last_activity', 'has_school_opps',
    // Opp wrapper fields (extracted to opps.json)
    'opp_owner', 'opportunity_name', 'intro_meeting_date',
    'created_date', 'last_modified', 'age', '_hasUploadedOpp',
    'metric_improvement_goal', 'metric_-_improvement_goal',
    'decision_criteria', 'decision_process', 'paper_process', 'implication_of_pain',
  ...
  return accounts.map(acct => {
    const cleaned = {};
    for (const key of Object.keys(acct)) {
      if (!RUNTIME_FIELDS.has(key)) cleaned[key] = acct[key];
    }
    return cleaned;
  });
}
...
export async function confirmMerge() {
  ...
  try {
    ...
    if (S.sfdcDataType === 'opps') {
      ...
      const strippedAccounts = stripRuntimeFields(S.ACCOUNT_DATA);
    ...
    if (S.mergeHasTypeSplit) {
      ...
      downloadJsonFile(stripRuntimeFields(S.pendingAccountMerge), 'accounts.json');
      ...
      if (isAccountType) {
        // Also download opps.json (extracted from merged data) for the prebuild pipeline
        const extractedOpps = extractOppsFromAccounts(S.pendingMergeData);
        downloadJsonFile(stripRuntimeFields(S.pendingMergeData), filename);
        await new Promise(resolve => setTimeout(resolve, 500));
        downloadJsonFile(extractedOpps, 'opps.json');
      } else {
