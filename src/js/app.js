...
// ============ DISTRICT KEY HELPER ============
// Includes state to prevent key collisions between districts with the same name in different states
...
// Pre-compute lowercase search fields to avoid repeated .toLowerCase() in filter loops
function precomputeSearchFields(records) {
  records.forEach(d => {
...
function toggleAccountListOverlay() {
  ...
  if (_elCountBadge) _elCountBadge.classList.toggle('active', accountListOpen);
...
function openAccountFromList(dKey) {
  let d = window.districtDataCache && window.districtDataCache[dKey];
  if (!d) {
    // Fallback: search markerLookup using districtKey for correct name+state matching
    const match = Object.entries(markerLookup).find(([k, v]) => districtKey(v.data) === dKey);
...
function openAccountModalByKey(dKey) {
...
function generateMeetingPrepByKey(dKey) {
  const d = window.districtDataCache && window.districtDataCache[dKey];
  if (!d) {
    console.error('District data not found for key:', dKey);
...
function buildLocationIndex() {
  ...
  [ACCOUNT_DATA, CUSTOMER_DATA].forEach(dataset => {
...
function inheritLocationData(record, locationIndex) {
...
function navigateToConflict(idx) {
  ...
  if (entry && entry.marker) {
    ...
    const d = ACCOUNT_DATA.find(a => a.name === c.name) || (window.districtDataCache && window.districtDataCache[dKey]);
